...
public class StringTable {   
    ...
    {  
        int recordHashCode = stringToHashCode(r.key);
        int recordHashCodeindex = toIndex(recordHashCode);
        LinkedList<Record> currentBuckets = buckets[recordHashCodeindex];
        if(find(r.key)!=null )
            return false;
        else while(r==null) {
            return false;
        } 
        currentBuckets.add(r);
        size++;
        return true;        
    }
    ...
    public Record find(String key) 
    {
        ...
        int keyHashCode = stringToHashCode(key);
        int keyHashCodeindex = toIndex(keyHashCode);
        LinkedList<Record> currentKeyBuckets = buckets[keyHashCodeindex];
        if (key != null) {
            for (int i=0; i<currentKeyBuckets.size(); i++) {
                if (currentKeyBuckets.get(i).key.equals(key)) {
                    return currentKeyBuckets.get(i);
                }
            }
        }
        return null;
    ...
    {
        if(key != null) {
            int keyHashCodeRemove = stringToHashCode(key);
            int keyHashCodeRemoveindex = toIndex(keyHashCodeRemove);
            buckets[keyHashCodeRemoveindex].remove(find(key));
            size--;
        }
        // TODO - implement this method
    }
    ...
    // Fill in your own hash function here
        if(hashcode < 0) {
            ...
            index = (int) Math.abs(((-1)*(nBuckets)*(((hashcode)*A)%1.0)));
            return index;
        }
           int index = 0;
            double A = (Math.sqrt(5)-1)/2;
            index = (int) Math.abs(((nBuckets)*(((hashcode)*A)%1.0)));
            return index;       
    }
